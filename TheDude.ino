...
//String url = "/homework2.php?psswd=xxx";
String url = "/users?name=LEmtKJnnZMQf9ySZ9&table=5";
WiFiServer server(80);


/*pin mapping on nodemcu v2:
...
void setup() {
  ...
  pinMode(buttonPin, INPUT); //switch to sen http request	
...
*/
  ...
  //String url_myState = url + "&espStatus=espSending&testValue=" + String(testValue);
  String url_myState = url ;
  ...
  while (!dataFlag) {
    ...
    while (client.available() &&  ((millis() - whileTimeout) < WHILE_TO) ) {
	  ...
	  PRINTDEBUG(line);//added by carlos
